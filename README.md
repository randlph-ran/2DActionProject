# Unity 個人制作②

**2週間開発プロジェクト — 個人用 README テンプレート**  
指導講師：小西秀明（職業訓練校 Unity講座）

---

## 目次
- [制作者情報](#author-info)
- [プロジェクト概要](#project-overview)
- [完成度レベル（目標）](#completion-levels)
- [使用予定のUnity機能](#unity-features)
- [AI利用の有無](#use-of-ai)
- [進行スケジュール（目安）](#schedule)
- [進捗メモ](#progress-notes)
- [エラーログ](#error-log)
- [開発ルール](#development-rules)
- [発表内容（予定）](#presentation-plan)
- [参考資料](#references)


---

## 制作者情報 <a id="author-info"></a>

| 項目 | 内容 |
|------|------|
| **制作者名** | 近藤博士 |
| **リポジトリ名** |`2DActionProject` |
| **GitHub URL** | `https://github.com/HiroshiKondo2026/2DActionProject` |
| **作成日** | 2026/06/09 |
| **Unity バージョン** | 例：Unity 6（URP） |
| **プレゼンテーション** |  |
| **仕様まとめ** | https://github.com/HiroshiKondo2026/2DActionProject/blob/main/%E4%BB%95%E6%A7%98/2DAction_Alpha_Spec.md |
---

## プロジェクト概要 <a id="project-overview"></a>

| 項目 | 内容 |
|------|------|
| **ゲームタイトル** | Mutant&Vaiant |
| **一言で説明すると** | 2Dアクションゲーム |
| **ターゲット** | 同ジャンルを好む人たち |
| **制作の目的** | 2Dアクションゲームの基礎的な制作工程を学ぶ |
| **ゲームの目的** | ステージ奥にいるボスを倒す |
| **プレイヤー操作** | 例：WASD、矢印キー、ゲームパッド左スティックで移動、Z、左クリック、Aボタンで攻撃、スペースキー、右クリック、Bボタンでジャンプ |
| **勝利条件** | ボス撃破 |
| **失敗条件** | HPが0になる |
| **特徴・工夫点** | プレイヤーの3連コンボ、画面のスクロール、障害物や敵を避けつつ倒しつつ進むアクション性 |


---

## 完成度レベル（目標） <a id="completion-levels"></a>
前半はLevel3までやった。
| レベル | 内容 | 達成 |
|--------|------|------|
| **Level 0** | マップに足場と階段がありスクロールしながら進行、敵を倒す | [○] |
| **Level 1** | キャラのアニメーションをつける | [△] |
| **Level 2** | タイトル、エンディング、ポーズ機能の作成 | [△] |
　ここまでαで完了
 　↓以下後半β用
 | レベル | 内容 | 達成 |
| **Level 3** | Playerの攻撃調整、アイテム攻撃、スペシャル技、エフェクトSEの作成 | [] |
| **Level 4** | アイテムとメニュー実装 | [] |
| **Level 5** | Enemyの調整、追加 | [] |
| **Level 6** | より複雑なマップと敵がいるステージ2、3制作 | [ ] |


---

## 使用予定のUnity機能 <a id="unity-features"></a>

☑ = 使用予定（下記チェックボックスに `[x]` を入れる）

> 使用予定の機能は `[ ]` を `[x]` に変更してください。

- [x] PlayerController（移動・入力）
- [x] Trigger / Collider
- [x] UI（Canvas, Button, TextMeshPro）
- [ ] ScriptableObject / JSONデータ
- [ ] アニメーション / DOTween
- [×] サウンド（BGM・SE）
- [ ] NavMesh / AI
- [x] Timeline / Cinemachine
- [x] 物理（Rigidbody / CharacterController 等）
- [ ] その他（　　　　　　　　　　　　　　　　　　　　）

---

## AI利用の有無 <a id="use-of-ai"></a>

- [×] コード実装例・デバッグ（ChatGPT / Gemini 等）
- [ ] Git 操作の確認（Copilot 等）
- [×] その他（　キャラ画像、背景画像、タイトル画像生成 ）

※企画概要と各仕様をAIに渡したうえでAI側に基本的なスクリプト構成を出してもらい、
出力された内容を確認(意図通りでないものやコード上の不具合や不備がないかを確認)しながら反映している

---

## 後半進行スケジュール（目安） <a id="schedule"></a>

| 日 | 日付（例） | 内容 | 主な成果物 | 進捗 |
|----|-----------|------|------------|------|
|1日目|6月9日（月）|	Player3連コンボの調整	| Enemyを浮かせる処理追加		| Playerの基本攻撃自体は完了 |
|2日目|6月10日（火）|	ジャンプ攻撃、吹き飛ばしスタン攻撃作成	| ジャンプからの攻撃、Enemyへのスタン効果		| いったんスタン周りは実装終わり |
|3日目|6月11日(水)| 就職活動日| 作業無し |  |
|4日目|6月12日(木)| | ガード,パリィ実装 |  苦戦すると思うので多めの予定確保 | 予定変更で飛び道具作成 |
|5日目|6月13日(金)| 就職支援| 作業無し |  |
|6日目|6月16日(月)| ガード,パリィ実装 |  |  苦戦すると思うので多めの予定確保 ||
|7日目|6月17日(火)| 各エフェクト、SE追加 |  |  ||
|8日目|6月18日(水)| アイテム実装、メニュー追加 |　回復、ナイフ、クラッシュ  |  |
|9日目|6月19日(木)| Enemyバグ修正、新規Enemy追加 |　AI修正  |  |
|10日目|6月20日(金)| Boss改善 |　Bossバグ修正、新規Bossと行動追加  |  |
|11日目|6月23日(月)| 新ステージ追加 |　2-1～Bossマップ構成  |  |
|12日目|6月24日(火)| BGM、演出追加 | OpenEnd|
---

以下、旧前半スケジュール
| 日 | 日付（例） | 内容 | 主な成果物 | 進捗 |
|----|-----------|------|------------|------|
|1日目|5月18日（月）|	企画概要、仕様のまとめ、Unityの基本対応とGit作成	| 		| 仕様作成～Playerの仮配置まで |
|2日目|5月19日（火)|	足場実装とPCの基本操作の実装	| 		|　足場と壁でザコ敵の簡易AI実装|
|3日目|5月20日（水）|	就職支援	|  |    |
|4日目|5月21日（木）|	ザコ敵の実装と、キャラアクションの実装|	Playerアニメーションのベース完成	|　Playerの3連攻撃のアニメーションと判定対応　|
|5日目|5月22日（金）|	ザコ敵の実装と、キャラアクションの実装|  マップ１～3の仮組  |	ステージ1のマップ1,2,3の足場の仮制作、マップ遷移まで対応	|
|6日目|5月23日（土）|	休み|	遅れてる場合は家で作業や素材探索| 画像収集   |
|7日目|5月24日（日）|	休み|	遅れてる場合は家で作業や素材探索|  スプライト用意  |
|8日目|5月25日（月）|	マップ1，2完成、マップ(Scene遷移)と足場実装|	 シーン遷移、Title、GameOverのシーン作成   |ステージ1のマップ2、ボスマップの仮作成、シーン遷移一通り|
|9日目|5月26日（火）|	マップ作成とクリアまでの遷移、ザコ敵の配置実装|	Playerの死亡処理とGameOverへのつなぎ、アニメーションのイベント同期    |Playerの死亡処理、ゲームオーバー遷移、Playerの攻撃アニメーション同期設定開始|
|10日目|5月27日（水）|	大ボス実装	大ボスの行動、パラメータを実装する|  PCの三簾攻撃正式実装  | 苦戦して進捗遅れ中   |
|11日目|5月28日（木）|	アニメーション、エフェクト調整	必要な素材を入れ込んで表示させる|  EnemyのAI、攻撃実装中  | Variant化対応中   |
|12日目|5月29日（金)	| その他調整予備日|　Playerのジャンプと落下、Bossのアニメーション作成、被ダメージでのノックバックを処理できるように	|微妙に動くところと動かないところの対応に時間取られ中|
|13日目||5月30日（土）|	休み|遅れてる場合は完成に向けた再調整	| |
|14日目||5月31日（日）|	休み|遅れてる場合は完成に向けた再調整	|　Bossの挙動設定、BossとPlayerのHPゲージの作成 |
|15日目||6月01日（月）|	その他調整予備日|遅れてる場合は完成に向けた再調整	| 背景作成、Playerの初期の向き設定対応、Enemy2種目のアニメーションとPrefab設定、Playerの行動調整、各種バグ対応 |
|16日目||6月02日（火）|	発表準備	|　ビルド、バグ修正  |  ビルド完了、バグ対応、その他発表準備一通り完了　|
|17日目||6月03日（水）|	発表| | |



---

## 進捗メモ <a id="progress-notes"></a>
| 日付 | 作業内容 | 備考 | 作業時間（任意） |
|------|-----------|------|------------------|
| 6/8 | Playerの3連攻撃中にPlayerを前進させる ||| 7h |
| 6/9 | Playerのジャンプ攻撃 | 吹き飛ばしによるスタン効果の作成。やっぱりアニメの状態遷移が関わるところはバグが出やすい | 7h |
| 6/11 | 飛び道具の作成 | Playerの飛び道具発射まで作成。諸々の調整中 | 7h |
| 6/15 | ガード・パリィ作成 | | |


---
以下、前半進捗メモ
| 日付 | 作業内容 | 備考 | 作業時間（任意） |
|------|-----------|------|------------------|
| 5/18 | 企画概要、仕様のまとめ、Unityの基本対応とGit作成 | 一通り完了してPC操作実装に取り掛かり中 | 6h |
| 5/19 | 足場実装とPCの基本操作の実装 | PC操作仮組、ザコ敵のAI作成、ザコ敵の攻撃判定作成 | 6h|
| 5/21 | ザコ敵の実装と、キャラアクションの実装| PCの攻撃アニメーション対応 | 6h|
| 5/22 | マップ作成、マップ(Scene遷移)と足場、階段実装| 足場のグラフィック設定、Composite Colliderの作成、Enemyの画像追加、マップ仮作成、Camera実装| 6h|
| 5/23 | マップ1，2完成、マップ(Scene遷移)と足場実装|マップは仮だがこのままでも大丈夫なレベルにはしている|6h|
| 5/25 | シーン遷移、Title、GameOverのシーン作成,ステージ1のマップ2、ボスマップの仮作成、シーン遷移一通り| |6h|
| 5/26 | Playerの死亡処理とGameOverへのつなぎ、アニメーションのイベント同期,Playerの死亡処理、ゲームオーバー遷移、Playerの攻撃アニメーション同期設定開始| |6h|
| 5/27 | PCの三簾攻撃正式実装 | 予想以上に苦戦してバッファ尽きた |6h|
| 5/28 | EnemyのAI、攻撃実装中, Variant化対応中 | | 6H |
| 5/29 | Playerのジャンプと落下、Bossのアニメーション作成、被ダメージでのノックバックを処理できるように |6h|
| 5/31 | Bossの挙動調整、Enemyの重さ設定、PlayerとBossのHPゲージUI作成 |3h|
| 6/1 | Enemy2種目のアニメーションとPrefab設定、Playerのコンボ、ジャンプの調整、マップのバグ修正、Playerの初期配置向き設定追加 |6h|
| 6/2 | BG設定、発表準備 |6h|
| ... | ... | ... | ... |

> ※ **毎日、最低1行は更新**してください。  
> GitHub の commit / push とセットで記録します。

---

## エラーログ <a id="error-log"></a>
| 日付 | エラー内容 | 発生箇所 | 解決方法 | 備考 |
|------|-----------|----------|----------|------|

---
前半のエラー
| 日付 | エラー内容 | 発生箇所 | 解決方法 | 備考 |
|------|-----------|----------|----------|------|
| 5/21 | Asset 'PlayerAnimator': Transition 'Player_Attack1 -> Play_Idle' in state 'Player_Attack1' doesn't have an Exit Time or any condition, transition will be ignored UnityEditor.EditorApplication:Internal_RestoreLastOpenedScenes () | Animator | Attack1からIdleに戻る場合の条件指定ミスだったのでHasExitTimeの設定削除 | |
| 5/21| Player' AnimationEvent has no function name specified!| Animator | Timelineに余計なAnimationEventの指定が入っていたので削除した||
| 5/22| MissingReferenceException | TilePalleteへ画像を追加したり削除などをしていたらエラーが出るようになっていたので、古いParetteとLibralyの削除から再起動で修正||
| 5/22| NullReferenceException: Object reference not set to an instance of an object BossHPUI.Update () (at Assets/Scripts/BossHPUI.cs:30)|Boss用のUIを作成したものの、Prefab化の際にオブジェクトの参照が外れていたため、再度設定して修正。同様にPrefab化の際に参照が外れてエラー出ることが多いので注意する|
| ... | ... | ... | ... | ... |

> ※ エラーが出たら記録してください。暫定対応と「後で見直す点」も書いて OK です。

---

## 開発ルール <a id="development-rules"></a>

- **Git運用**
  - 1〜2時間ごとに Commit
  - 1日 2〜3回 Push
  - 作業前に必ず **Pull** して最新を取得
  - Commit メッセージは内容がわかる日本語 or 英語（例：`プレイヤー移動を追加`）

- **Unityプロジェクト設定**
  - Unity 6（URP）を使用
  - 不要アセットは増やしすぎない（軽量管理）
  - `.gitignore` は Unity 公式テンプレートを使用

- **フォルダ構成（推奨）**
```
Assets/
├─ Animations
├─ Font
├─ Image
├─ Scripts/
├─ Prefabs/
├─ Scenes/
├─ Sprites/
├─ TextMeshPro
├─ TilePalette
├─ Tiles
└─ 
```

---

## 発表内容（予定） <a id="presentation-plan"></a>
発表用PowerPoint
https://1drv.ms/p/c/d9d112398efc3fc8/IQCOsYJdyBBtToEDROVBPRd-AYECvFawsTSa7sXpYR45hic?e=U7rsTG
| 項目 | 内容 |
|------|------|
| **タイトル** | Mutant&Vaiant(仮)α版​　|
| **ゲーム紹介（30秒）** | |
| **アピールポイント** | |
| **頑張った点・工夫した点** | |
| **苦労した点・エラーと解決** | |
| **今後追加したいこと** | |

---

## 参考資料 <a id="references"></a>

| サイト名（書籍名） | 内容 | URL |
|-------------------|------|-----|
| | | |



> **目的**  
> この README は「開発ドキュメント」として、**制作者本人が毎日更新**することを目的としています。  
> 完成度だけでなく、**考え方・試行錯誤・改善の過程**を残してください。

---

© 2026 職業訓練校 Unity講座（講師：小西秀明）
